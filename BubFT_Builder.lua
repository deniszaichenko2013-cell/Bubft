--[[
    ...
    Твоя картинка уже вшита
]]

local IMAGE_URL = "https://cdn.discordapp.com/attachments/1526727012276764864/1531811465630322821/images.jpg"
local RESOLUTION = 32
...
local function build(colors, w, h, startPos)
    ...
    for y = 1, h do
        for x = 1, w do
            local px = startPos.X + (x-1) * BLOCK_SIZE
            local pz = startPos.Z + (y-1) * BLOCK_SIZE
            local pos = Vector3.new(px, startPos.Y, pz)
            ...
            if existing then
                ...
                placed = placed + 1
                task.wait(0.3)
                local newB = findBlock(pos, BLOCK_SIZE * 0.8)
                if newB then table.insert(placedBlocks, {pos = pos, block = newB}) end
            end
    ...
    for y = 1, h do
        for x = 1, w do
            ...
            local block = nil
            for _, entry in pairs(placedBlocks) do
                if (entry.pos - pos).Magnitude < 1 then block = entry.block; break end
            end
...
local function createGUI()
    ...
    buildBtn.Position = UDim2.new(0.05, 0, 0, 85)
    ...
    buildBtn.MouseButton1Click:Connect(function()
        ...
        spawn(function()
            ...
            img.Size = UDim2.new(0, RESOLUTION, 0, RESOLUTION)
            img.Position = UDim2.new(0, -1000, 0, -1000)
            img.Image = IMAGE_URL
            img.Parent = sg
            
            repeat task.wait() until img.IsLoaded
...
if game:GetService("CoreGui"):FindFirstChild("BubFT") then
    game:GetService("CoreGui").BubFT:Destroy()
end

createGUI()
print("BubFT loaded! Press BUILD in GUI.")
